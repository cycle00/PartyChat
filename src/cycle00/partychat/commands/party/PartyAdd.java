...
public class PartyAdd {
    ...
    public static void execute(Player player, String newPlayer) {
        ...
        TextComponent msg = new TextComponent(Utils.chat("&aClick here to join &f" + player + "'s&a party."));
        msg.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/p join " + player));
        ...
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(PartyChat.instance, () -> {
            if (party.pendingInvitations.contains(invitee)) {
                ...
                player.sendMessage(Utils.chat("&cThe party invite to &f" + invitee + "&c has expired."));
                invitee.sendMessage(Utils.chat("&cThe party invite from &f" + player + "&c has expired."));
            }
        }, 1200);
    }
}
